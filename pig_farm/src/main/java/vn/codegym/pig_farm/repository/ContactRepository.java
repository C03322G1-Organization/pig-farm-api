...
public interface ContactRepository extends JpaRepository<Contact, Integer> {
    ...
    @Query(value = "select id, `name`, email, phone, address, content,`date`, is_deleted from contact where is_deleted = 0 and `name` like :nameSearch and content like :contentSearch", nativeQuery = true,
            countQuery = "select count(*) from (select id, `name`, email, phone, address, content,`date`, is_deleted from contact where is_deleted = 0 and `name` like :nameSearch and content like :contentSearch) as aaaa")
